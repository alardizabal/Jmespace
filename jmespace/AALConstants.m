//
//  AALConstants.m
//  jmespace
//
//  Created by Albert Lardizabal on 8/6/14.
//  Copyright (c) 2014 Albert Lardizabal. All rights reserved.
//

#import "AALConstants.h"

@implementation AALConstants

NSString *const kGOOGLE_API_KEY = @"AIzaSyAUfnxVJenu9mQSwFCd_h-dkPCX2LnExeM";
NSString *const kPARSE_APP_ID = @"4Gnu6iW5BdhOnccUzHGFzRQb9dLf0EcMCCxHjJzH";
NSString *const kPARSE_CLIENT_KEY = @"z7ISJ9SMQrYEf0lrz9euH9e5tT9ffO3VXJpGuMg0";
NSString *const kFB_APP_ID = @"1587985351428891";
NSString *const kFB_APP_SECRET = @"c0e42bae11ae1eeecf2c0eac97b34a23";

NSString *const kGPLACES_API_URL = @"https://maps.googleapis.com/maps/api/place/nearbysearch/json";
NSString *const kGPLACES_PHOTOS_API_URL = @"https://maps.googleapis.com/maps/api/place/photo";

@end
